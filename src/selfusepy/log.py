...
def __delta_base__(fmt, delta: int, timestamp):
    return datetime.fromtimestamp(
        timestamp, timezone(timedelta(hours=delta))
    ).timetuple()
...
class Logger(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        formatter.time_offset = time_offset
        self.logger.setLevel(
            logging.NOTSET
        )  # 设置日志级别为notset, 所有的log都可以打印出来
        ...
        if filename is not None:
            ...
            if "encoding" in kwargs.keys():
                ...
                th = handlers.TimedRotatingFileHandler(
                    filename=filename, encoding="utf-8", **kwargs
                )
...
class LoggerFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord):
        ...
        i: int = 0
        while (
            self._s_len(l) + file_name.__len__() + record.funcName.__len__() > 50
        ):  # 如果超出了长度再进行缩减操作
            if (
                i >= l.__len__()
            ):  # 实在太长了缩减不了, 就算了, 需要保证最后的文件名与函数名的完整
                break
            l[i] = l[i][0]
            i += 1
