...
import selfusepy
from bilibili.AV import AV
from bilibili.AVDO import AVInfoDO, AVStatDO
from bilibili.db import DBSession
...
if __name__ == '__main__':
  ...
  while True:
    ...
    for item in obj.onlineList:
      ...
      #
      # print(jsonTemp['aid'])
      exist = session.query(AVInfoDO).filter_by(aid = avInfo.aid).first()
      if not exist:
        try:
          session.add(avInfo)
          session.add(avStat)
